export class ProductsListPage {
  ...
  async addItemToCart(itemName) {
    await this.page
      .locator(
        `//*[@data-test='inventory-item-name' and text() = '${itemName}']/following::button[text() = 'Add to cart']`
      )
      .first()
      .click();
  }
  ...
  async capturePriceOfItem(itemName) {
    const priceLocator = await this.page
      .locator(
        `//*[@data-test='inventory-item-name' and text()='${itemName}']/following::div[@class='inventory_item_price']`
      )
      .first();
